...
public class GroundTile : MonoBehaviour
{
    ...
    public GameObject forcefield_1,forcefield_duck,forcefield_jump,gateOrange_1,gateBlue_1,turretOrange,turretBlue,disc_coin,health_pack;

    void SpawnObstacleGate()
    {
        int obstacleType = Random.Range(0,5); // obstacleType = 0(means different forcefields), = 1(means different gates), = 2(means turrets), = 3(means coins)
    ...
    void SpawnHealth()
    {
        ...
        spawnPoint = transform.GetChild(obstacleIndex).transform;
        spawnPoint.position = new Vector3(spawnPoint.position.x,3f,spawnPoint.position.z);
        Instantiate(health_pack,spawnPoint.position,Quaternion.identity,transform); 
